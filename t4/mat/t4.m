...
fid = fopen("confirmation_data_tab.tex","w")
...
fprintf(fid, "$V_{collector}$& %f V \\\\ \\hline \n", VO1)
fprintf(fid, "$V_{emitter}$& %f V \\\\ \\hline \n", VE1)
fclose(fid)
...
fprintf(fid, "$V_{vcc}$& 12 V \\\\ \\hline \n")
fprintf(fid, "$V_{collector}$& %f V \\\\ \\hline \n",VO1)
fprintf(fid, "$V_{emitter2}$& %f V \\\\ \\hline \n", VO2)
...
aux1 = 1/(rpi2+ZO1);
aux2 = (gm2*rpi2)/(rpi2+ZO1);
AV = (aux1+aux2)/(aux1+ge2+go2+aux2)*AV1;
AVdB = 20*log10(abs(AV));

fid = fopen("total_tab.tex","w")
fprintf(fid, "Input impedance& %f Ohm \\\\ \\hline \n", ZI)
fprintf(fid, "Output impedance& %f Ohm \\\\ \\hline \n", ZO)
fprintf(fid, "Gain& %f \\\\ \\hline \n", AV)
fprintf(fid, "Gain(dB)& %f dB \\\\ \\hline \n", AVdB)
...
for i=1:length(f)
C = [0, 0, -gpi2-gm2, gR_out+gpi2+gm2+go2+j*2*pi*f(i)*Co, -j*2*pi*f(i)*Co, 0 ; gm1, -gm1, go1+gRC+gpi2, -go1-gpi2, 0, 0 ; -gpi1-gm1, gE1+gpi1+gm1+go1+j*2*pi*f(i)*Cb, -go1, 0, 0, 0 ; j*2*pi*f(i)*Ci+gpi1, -gpi1, 0, 0, 0, -j*2*pi*f(i)*Ci ; -j*2*pi*f(i)*Ci, 0, 0, 0, 0, (1/RS)+j*2*pi*f(i)*Ci ; 0, 0, 0, -j*2*pi*f(i)*Co, j*2*pi*f(i)*Co+(1/R_L), 0];
D = [0 ; 0 ; 0 ; 0 ; 0.01/RS; 0];
R = C\D;
T(i) = R(5)/R(6);
endfor
